...
export class D1Store implements Store {
  ...
  async persist(): Promise<void> {
    ...
    diff(this.snapshots.pigeons, w.pigeons, (p) => p.id, {
      upsert: (p) =>
        db.prepare(
          'INSERT OR REPLACE INTO pigeons (id, owner_id, name, sex, birth_week, speed, endurance, orientation, libido, form, health, experience, sire_id, dam_id, for_sale, price, created_at_week, retired, ailment, in_infirmary, races, ever_ailed, coached) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        ).bind(
